...
/// Generates the content of a file defining the messages of the given errors keyed by their error
/// code, sorted by it, and writes it to the file at `file_path`.
///
/// # Errors
/// Returns an error if two different error messages hash to the same error code.
pub fn generate_error_message_table(
    file_path: impl AsRef<Path>,
    errors: &[NamedError],
) -> Result<()> {
    let mut messages: BTreeMap<u64, &str> = BTreeMap::new();
    for NamedError { name, message } in errors {
        let code = error_code_from_msg(message).as_canonical_u64();
        if let Some(existing_message) = messages.insert(code, message.as_str())
            && existing_message != message
        {
            return Err(Report::msg(format!(
                "error code {code} of `ERR_{name}` collides with the code of the different error message `{existing_message}`"
            )));
        }
    }
...
/// The messages of all MASM errors of this crate, keyed by their error code and sorted by it.
pub const MASM_ERROR_MESSAGES: [(u64, &str); {}] = [",
        messages.len()
    )
    .into_diagnostic()?;

    for (code, message) in messages {
        writeln!(output, "    ({code}, {message:?}),").into_diagnostic()?;
    }
