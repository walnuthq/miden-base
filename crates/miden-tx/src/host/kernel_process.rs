use miden_processor::{Felt, ProcessorState};
use miden_protocol::{PrimeCharacteristicRing, PrimeField64, Word};
...
impl<'a> TransactionKernelProcess for ProcessorState<'a> {
    fn get_active_account_ptr(&self) -> Result<u32, TransactionKernelError> {
        ...
        let account_stack_top_ptr = u32::try_from(account_stack_top_ptr.as_canonical_u64()).map_err(|_| {
    ...
    fn has_merkle_path<const TREE_DEPTH: u8>(
        ...
        self.advice_provider()
            .has_merkle_path(root, Felt::from_u8(TREE_DEPTH), leaf_index)
            .map_err(|err| {
                TransactionKernelError::other_with_source(
