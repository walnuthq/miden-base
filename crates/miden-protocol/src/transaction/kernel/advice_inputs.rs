...
use crate::{EMPTY_WORD, Felt, PrimeCharacteristicRing, Word, ZERO};
...
impl TransactionAdviceInputs {
    ...
    fn build_stack(&mut self, tx_inputs: &TransactionInputs) {
        ...
        self.extend_stack([
            ...
            Felt::from_u32(header.version()),
            Felt::from_u32(header.timestamp()),
            ZERO,
        ]);
        self.extend_stack([
            header.fee_parameters().native_asset_id().suffix(),
            header.fee_parameters().native_asset_id().prefix().as_felt(),
            Felt::from_u32(header.fee_parameters().verification_base_fee()),
            ZERO,
        ]);
        ...
        self.extend_stack([Felt::from_u16(tx_inputs.input_notes().num_notes())]);
    ...
    fn add_account(&mut self, account: &PartialAccount) {
        ...
        self.extend_map(account.storage().leaves().map(|leaf| (leaf.hash(), leaf.to_elements().collect::<alloc::vec::Vec<_>>())));
        ...
        self.extend_map(account.vault().leaves().map(|leaf| (leaf.hash(), leaf.to_elements().collect::<alloc::vec::Vec<_>>())));
    }
    ...
    fn add_account_witness(&mut self, witness: &AccountWitness) {
        ...
        self.add_map_entry(leaf.hash(), leaf.to_elements().collect::<alloc::vec::Vec<_>>());
    ...
    fn add_input_notes(&mut self, tx_inputs: &TransactionInputs) {
        ...
        for input_note in tx_inputs.input_notes().iter() {
            ...
            note_data.push(Felt::from_u16(recipient.storage().num_items()));
            note_data.push(Felt::from_u32(assets.num_assets() as u32));
            ...
            match input_note {
                InputNote::Authenticated { note, proof } => {
                    ...
                    note_data.push(Felt::from_u16(proof.location().node_index_in_block()));
                },
